...
static BOOL MainMenu(PBOOT_CFG pBootCfg)
{
	...
	EdbgOutputDebugString("\t\t%B : Command Type\r\n", pBSPArgs->CMD_wType);
	EdbgOutputDebugString("\t\t%B.%B : Command Version\r\n", pBSPArgs->CMD_bMajor, pBSPArgs->CMD_bMinor);
	EdbgOutputDebugString("\t\t%X : Waveform File Size\r\n", pBSPArgs->WFM_dwFileSize);
	EdbgOutputDebugString("\t\t%X : Waveform Serial Number\r\n", pBSPArgs->WFM_dwSerialNumber);
	EdbgOutputDebugString("\t\t%B : Waveform Run Type\r\n", pBSPArgs->WFM_bRunType);
	EdbgOutputDebugString("\t\t\t(0x00=[B]aseline,0x01=[T]est/trial,0x02=[P]roduction,0x03=[Q]ualification,0x04=V110[A])\r\n");
	EdbgOutputDebugString("\t\t%B : Waveform FPL Platform\r\n", pBSPArgs->WFM_bFPLPlatform);
	EdbgOutputDebugString("\t\t\t(0x00=2.0, 0x01=2.1, 0x02=2.3; 0x03=Vizplex 110, 0x04=V110A; other values undefined)\r\n");
	EdbgOutputDebugString("\t\t%B : Waveform FPL Lot\r\n", pBSPArgs->WFM_wFPLLot);
	EdbgOutputDebugString("\t\t%B : Waveform Mode Version(0x01 -> 0 INIT, 1 DU, 2 GC16, 3 GC4)\r\n", pBSPArgs->WFM_bModeVersion);
	EdbgOutputDebugString("\t\t%B : Waveform Version\r\n", pBSPArgs->WFM_bWaveformVersion);
	EdbgOutputDebugString("\t\t%B : Waveform Subversion\r\n", pBSPArgs->WFM_bWaveformSubVersion);
	EdbgOutputDebugString("\t\t%B : Waveform Type(0x0B=TE, 0x0E=WE; other values undefined)\r\n", pBSPArgs->WFM_bWaveformType);
	EdbgOutputDebugString("\t\t%B : Waveform FPL Size(0x32=5\", 0x3C=6\", 0x50=8\", 0x61=9.7\")\r\n", pBSPArgs->WFM_bFPLSize);
	EdbgOutputDebugString("\t\t%B : Waveform MFG Code(0x01=PVI,0x02=LGD,0x03=P/H)\r\n", pBSPArgs->WFM_bMFGCode);
