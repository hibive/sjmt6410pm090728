...
S3C6410Disp::S3C6410Disp()
{
	...
	if (DispDrvrPhysicalFrameBuffer == NULL)
	{
		...
		{
			RETAILMSG(1, (TEXT("S3C6410Disp::S3C6410Disp: Error allocating GPESurf.\r\n")));
			return;
		}
	...
	{
		RETAILMSG(1, (TEXT("S3C6410Disp::S3C6410Disp: Error (DispDrvrPhysicalFrameBuffer != NULL)\r\n")));
	}
}

SCODE S3C6410Disp::SetMode(int modeId, HPALETTE *pPalette)
{
	if (modeId != 0)
		return E_INVALIDARG;
	...
	if (pPalette)
		*pPalette = EngCreatePalette(PAL_INDEXED, DispDrvr_palSize, (ULONG *)DispDrvr_palette, 0, 0, 0);
...
SCODE S3C6410Disp::GetModeInfo(GPEMode *pMode, int modeNo)
{
	if (modeNo != 0)
		return E_INVALIDARG;
...
SCODE S3C6410Disp::MovePointer(int x, int y)
{
	return S_OK;
}

void S3C6410Disp::WaitForNotBusy()
{
	return;
}

int S3C6410Disp::IsBusy()
{
	return 0;	// Never busy as there is no acceleration
}

void S3C6410Disp::GetPhysicalVideoMemory(unsigned long *pPhysicalMemoryBase, unsigned long *pVideoMemorySize)
{
...
SCODE S3C6410Disp::AllocSurface(GPESurf **ppSurf, int width, int height, EGPEFormat format, int surfaceFlags)
{
	if (surfaceFlags & GPE_REQUIRE_VIDEO_MEMORY)
		return E_OUTOFMEMORY;	// Can't allocate video-memory surfaces in the S3C6410Disp environment
...
SCODE S3C6410Disp::WrappedEmulatedLine(GPELineParms *pParms)
{
...
SCODE S3C6410Disp::Line(GPELineParms *pLineParms, EGPEPhase phase)
{
	DEBUGMSG(GPE_ZONE_LINE, (TEXT("S3C6410Disp::Line\r\n")));
...
SCODE S3C6410Disp::WrappedEmulatedBlt(GPEBltParms *pParms)
{
...
SCODE S3C6410Disp::BltPrepare(GPEBltParms *pBltParms)
{
	DEBUGMSG(GPE_ZONE_LINE, (TEXT("S3C6410Disp::BltPrepare\r\n")));

	if ((pBltParms->pDst != m_pPrimarySurface) || (DispDrvrPhysicalFrameBuffer != NULL))
		pBltParms->pBlt = &GPE::EmulatedBlt;
	else
		pBltParms->pBlt = (SCODE (GPE::*)(struct GPEBltParms *))&S3C6410Disp::WrappedEmulatedBlt;

	return S_OK;
}

// This function would be used to undo the setting of clip registers etc
SCODE S3C6410Disp::BltComplete(GPEBltParms *pBltParms)
{
	return S_OK;
}

int S3C6410Disp::InVBlank()
{
	return 0;
}

SCODE S3C6410Disp::SetPalette(const PALETTEENTRY *src, unsigned short firstEntry, unsigned short numEntries)
{
	return S_OK;
}
...
ULONG S3C6410Disp::DrvEscape(SURFOBJ *pso, ULONG iEsc, ULONG cjIn, PVOID pvIn, ULONG cjOut, PVOID pvOut)
{
