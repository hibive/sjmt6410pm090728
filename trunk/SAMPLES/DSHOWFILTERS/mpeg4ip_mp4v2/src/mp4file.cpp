...
void MP4File::Open(const char* fmode)
{
    ...
    if (m_mode == 'r') {
#ifdef SIMON_CHANGED    // Simon Chun
        fseek(m_pFile, 0L, SEEK_END);
        m_orgFileSize = m_fileSize = ftell(m_pFile);
...
#ifdef SIMON_CHANGED
    wchar_t   tmpfile_wchar[256];

    GetTempFileName(L".", // dir. for temp. files 
                    L"mp4",                // temp. filename prefix 
                    0,                    // create unique name 
                    tmpfile_wchar);        // buffer for name 
    wchar_t2char(tmpfile_wchar, m_tempFileName);
...
#ifdef SIMON_CHANGED
    wchar_t   old_wchar[256], new_wchar[256];

    char2wchar_t(newFileName, new_wchar);
    char2wchar_t(oldFileName, old_wchar);
    DeleteFile(new_wchar);
    if (MoveFile(old_wchar, new_wchar))
        rc = 0;
    else
        rc = 1;
#else
    ...
    MP4Property** ppProperty, u_int32_t* pIndex)
{
...
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s - %s", "MP4File::FindIntegerProperty", "no such property", name);
        RaiseException(1, 0, 0, NULL);
#else
    ...
    default:
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s", "MP4File::FindIntegerProperty", "type mismatch - property");
        RaiseException(1, 0, 0, NULL);
#else
    ...
    MP4Property** ppProperty, u_int32_t* pIndex)
{
    if (!FindProperty(name, ppProperty, pIndex)) {
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s - %s", "MP4File::FindFloatProperty", "no such property", name);
        RaiseException(1, 0, 0, NULL);
#else
...
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s", "MP4File::FindFloatProperty", "type mismatch - property");
        RaiseException(1, 0, 0, NULL);
#else
    ...
    MP4Property** ppProperty, u_int32_t* pIndex)
{
    if (!FindProperty(name, ppProperty, pIndex)) {
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s - %s", "MP4File::FindStringProperty", "no such property", name);
        RaiseException(1, 0, 0, NULL);
#else
...
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s", "MP4File::FindStringProperty", "type mismatch - property");
        RaiseException(1, 0, 0, NULL);
#else
    ...
    MP4Property** ppProperty, u_int32_t* pIndex)
{
    if (!FindProperty(name, ppProperty, pIndex)) {
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s - %s", "MP4File::FindBytesProperty", "no such property", name);
        RaiseException(1, 0, 0, NULL);
#else
...
#ifdef SIMON_CHANGED
        printf("\n[%s]  %s", "MP4File::FindBytesProperty", "type mismatch - property");
        RaiseException(1, 0, 0, NULL);
#else
...
MP4TrackId MP4File::AddODTrack()
{
    ...
    if (m_odTrackId != MP4_INVALID_TRACK_ID) {
//        throw new MP4Error("object description track already exists", "AddObjectDescriptionTrack");
        RaiseException(1, 0, 0, NULL);
    }
...
#ifdef SIMON_CHANGED
    printf("\n[%s]  %s - %d", "FindTrackIndex", "Track id %d doesn't exist", trackId);
    RaiseException(1, 0, 0, NULL);
#else
...
#ifdef SIMON_CHANGED
    printf("\n[%s]  %s - %d", "FindTrakAtomIndex", "Track id doesn't exist", trackId);
    RaiseException(1, 0, 0, NULL);
#else
