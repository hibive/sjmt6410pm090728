...
EXIT:
    ...
    VOID
                ...
                )
        {
            ...
            if (fEnable) {
                m_isSDIOInterrupt = FALSE;
                wIntSignalEn |= NORMAL_INT_SIGNAL_CARD_INT;
            } else {
                m_isSDIOInterrupt = TRUE;
                wIntSignalEn &= (~NORMAL_INT_SIGNAL_CARD_INT);      
            }
    ...
    VOID 
                ...
                )
        {
            ...
            {
                DEBUGMSG(SDHC_CLOCK_ZONE,(TEXT("[HSMMC0] Turn OFF the F/B delay control.\r\n")));
                dwControlValue &= ~(0x3<<14); // Turn off the feedback clock delay
            }
            else
            {
                DEBUGMSG(SDHC_CLOCK_ZONE,(TEXT("[HSMMC0] Turn ON the F/B delay control.\r\n")));
    ...
    VOID 
                ...
                )
        {
            ...
            if (fSuccess) {
                ...
                RETAILMSG(TRUE, (_T("[HSMMC0] %s Timeout waiting for CLOCK_STABLE\r\n"), pszFname));
            }
        }
    ...
    DWORD
                ...
                )
        {
            ...
            // Below code line is needed for supporting both SD Memory and MMC.
    ...
    BOOL
        ...
        BOOL fRet = FALSE;
                ...
                fRet = (pIOPreg->GPNDAT & (0x1<<12)) ? 1 : 0; // It is always "0" because WP pin is not connected for now and the status of line is floating.
                MmUnmapIoSpace((PVOID)pIOPreg, sizeof(S3C6410_GPIO_REG));
                return fRet;
#else
