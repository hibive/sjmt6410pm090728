        ...
        )
{
    ...
    if (fRet) { 
	    ...
	    if (v_gBspArgs == NULL)
	    {
	        ...
	        if (v_gBspArgs == NULL)
	        {
                RETAILMSG(TRUE, (TEXT("[HSMMC2] HSMMC MmMapIoSpace: FAILED\r\n")));
	            if (v_gBspArgs)
	            {
        ...
        )
{
    ...
    if (0 != pInterface->InterfaceModeEx.bit.hsmmc8Bit) {
        ...
        RETAILMSG(TRUE, 
                (TEXT("[HSMMC2] Setting for 8 bit mode , Clock Rate = %d Hz\n"),pInterface->ClockRate));
        ...
        RETAILMSG(TRUE, 
                (TEXT("[HSMMC2] Setting for 1 bit mode , Clock Rate = %d Hz\n"),pInterface->ClockRate));        
        bHostCtr = 0;
        m_f4BitMode = FALSE;
        ...
        RETAILMSG(TRUE, 
                (TEXT("[HSMMC2] Setting for 4 bit mode , Clock Rate = %d Hz\n"),pInterface->ClockRate));
...
#else
// New Interrupt handler function can process factors on new card detect interrupt of HSMMC ch2 on SMDK6410.
VOID CSDHCSlotBase::HandleInterrupt(SDSLOT_INT_TYPE intType)
#endif
{
    Validate();
#ifdef _SMDK6410_CH2_EXTCD_
    // in case of it is occurred a card detect interrupt of HSMMC ch2 on SMDK6410
...
#ifdef _SMDK6410_CH2_EXTCD_
// New function can detect whether card is presented of HSMMC ch2 on SMDK6410.
        ...
        )
{
    ...
    if (wErrorStatus == 0)
    {
        DEBUGMSG(SDCARD_ZONE_ERROR, 
                (TEXT("[HSMMC2] HandleErrors - ERROR INT STATUS=0x%02X\n"), wErrorStatus));
        return;
    }
    ...
    if (pRequest) {
        ...
        RETAILMSG(TRUE, (TEXT("[HSMMC2] HandleErrors - ERR CMD:%d : "), pRequest->CommandCode));
    }
    ...
    if (wErrorStatus) {
        if ( wErrorStatus & ERR_INT_STATUS_CMD_TIMEOUT ) {
            ...
            switch(pRequest->CommandCode) {
                ...
                case 8 : RETAILMSG(TRUE, (TEXT("If the card is not SD SPEC 2.0, CMD 8 does not work in reason.\n")));
                    break;
                default :
                    RETAILMSG(TRUE, (TEXT("[HSMMC2] HandleErrors - CMD Timeout Error...\n")));
                    break;
            }
        }

        if ( wErrorStatus & ERR_INT_STATUS_CMD_CRC ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - CMD CRC Error...\r\n")));            
        ...
        if ( wErrorStatus & ERR_INT_STATUS_CMD_ENDBIT ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - CMD ENDBIT Error...\r\n")));                        
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
        }

        if ( wErrorStatus & ERR_INT_STATUS_CMD_INDEX ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - CMD INDEX Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_TIMEOUT ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - DAT Timeout Error...\r\n")));            
            status = SD_API_STATUS_DATA_TIMEOUT;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_CRC ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - Data CRC Error...\r\n")));              
            status = SD_API_STATUS_CRC_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_ENDBIT ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - DAT END BIT Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_BUS_POWER ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - Bus Power Error...\r\n")));
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_AUTOCMD12 ) {
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - AUTOCMD12 Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if (wErrorStatus & ERR_INT_STATUS_ADMA) { // ADMA Error
            RETAILMSG(TRUE,(TEXT("[HSMMC2] HandleErrors - ADMA Error...\r\n")));
            if (m_SlotDma && pRequest ) {
                m_SlotDma->DMANotifyEvent(*pRequest, DMA_ERROR_OCCOR );
            }
            else {
                DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC2] HandleErrors - ADMA Error without DMA Enabled (0x%x). Resetting CMD line.\r\n"),
                            wErrorStatus));
            }
        }
        ...
        if (IS_CMD_LINE_ERROR(wErrorStatus)) {
            // Reset CMD line
            DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC2] HandleErrors - Command line error (0x%x). Resetting CMD line.\r\n"),
                        wErrorStatus));
        ...
        if (IS_DAT_LINE_ERROR(wErrorStatus)) {
            // Reset DAT line
            DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC2] HandleErrors - Data line error (0x%x). Resetting DAT line.\r\n"),
                        wErrorStatus));
            ...
            do {
                ...
                if(RetryCount == 0) {
                    DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC2] HandleErrors - DAT line error Recovery Timeout...\r\n")));
                }
            } while(RetryCount != 0);
        }
        ...
        )
{
    ...
    {
        DEBUGMSG(SDHC_CLOCK_ZONE,(TEXT("[HSMMC2] Turn OFF the F/B delay control.\r\n")));
        dwControlValue &= ~(0x3<<14); // Turn off the feedback clock delay
    }
    else
    {
        DEBUGMSG(SDHC_CLOCK_ZONE,(TEXT("[HSMMC2] Turn ON the F/B delay control.\r\n")));
        ...
        )
{
    ...
    if (fSuccess) {
        ...
        RETAILMSG(TRUE, (_T("[HSMMC2] %s Timeout waiting for CLOCK_STABLE\r\n"), pszFname));
    }
}
...
CSDHCSlotBase::IsWriteProtected() {
    ...
    // On the CPU B'd Rev 0.0 of SMDK6410 , Write protect is detected with GPIO_N[12]
    BOOL fRet = FALSE;
    volatile S3C6410_GPIO_REG *pIOPreg = NULL;
    PHYSICAL_ADDRESS    ioPhysicalBase = {0,0};
    ioPhysicalBase.LowPart = S3C6410_BASE_REG_PA_GPIO;
    pIOPreg = (volatile S3C6410_GPIO_REG *)MmMapIoSpace(ioPhysicalBase, sizeof(S3C6410_GPIO_REG), FALSE);
    if (pIOPreg == NULL) {
        RETAILMSG (1,(TEXT("[HSMMC2] GPIO registers is *NOT* mapped.\n")));
        return FALSE;
    }
    fRet = (pIOPreg->GPNDAT & (0x1<<14)) ? 1 : 0; // It is always "0" because WP pin is not connected for now and the status of line is floating.
    MmUnmapIoSpace((PVOID)pIOPreg, sizeof(S3C6410_GPIO_REG));
    return fRet;
#else
