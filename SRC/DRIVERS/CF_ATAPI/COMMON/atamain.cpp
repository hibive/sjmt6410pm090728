    ...
    PDWORD pOverlapped)
{
    ...
    {
        RETAILMSG(1, (TEXT("DSK_IOControl: CeOpenCallerBuffer failed in ATAPI for IN buf.\r\n")));
        return FALSE;
    }
    if(FAILED(CeOpenCallerBuffer(&pMarshalledOutBuf, pOutBuf, nOutBufSize, ARG_O_PTR, TRUE)))
    {
        RETAILMSG(1, (TEXT("DSK_IOControl: CeOpenCallerBuffer failed in ATAPI for OUT buf.\r\n")));
        return FALSE;
    }
    ...
    if (dwIoControlCode == DISK_IOCTL_INITIALIZED) {
        fRet = pDisk->PostInit((PPOST_INIT_BUF)pMarshalledInBuf);
    }
    else {
        ...
        IOReq.pInBuf = pMarshalledInBuf;
        IOReq.dwInBufSize = nInBufSize;
        IOReq.pOutBuf = pMarshalledOutBuf;
    ...
    {
        RETAILMSG(1, (TEXT("DSK_IOControl: CeCloseCallerBuffer failed in ATAPI for IN buf.\r\n")));
        return FALSE;
    }
    if(FAILED(CeCloseCallerBuffer(pMarshalledOutBuf, pOutBuf, nOutBufSize, ARG_O_PTR)))
    {
        RETAILMSG(1, (TEXT("DSK_IOControl: CeCloseCallerBuffer failed in ATAPI for OUT buf.\r\n")));
        return FALSE;
    }
