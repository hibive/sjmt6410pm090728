    ...
    )
{
    ...
    else if (dwEndpoint < ENDPOINT_COUNT)
    {
        ...
        for(DWORD depth=0;depth<PIPELINED_FIFO_DEPTH;depth++)
        {
            pContext->pVAddrEP[dwEndpoint][depth] = (PDWORD)AllocPhysMem(64*1024, PAGE_READWRITE, 0, 0, &pContext->pPAddrEP[dwEndpoint][depth]);
            if(pContext->pVAddrEP[dwEndpoint][depth] == NULL)
            {
                RETAILMSG(UFN_ZONE_WARNING,(_T("[UFNPDD] %d EP Transfer Buffer Allocation is failed\r\n"), dwEndpoint));
                peps->bInitialized = FALSE;
            }
            else 
            {
                memset(pContext->pVAddrEP[dwEndpoint][depth], 0, 64*1024);
            }
        }
#endif
    ...
    )
{
    ...
    {
        WriteReg(DOEPTSIZ0, 1<<SETUP_PKT_CNT_IDX | 1<<PACKET_COUTNT_IDX | 0);
        WriteReg(DOEPCTL0, EP_ENABLE | CLEAR_NAK | EP0_MAX_PK_SIZ);
