...
//		for ( i = 0; i < 12; i++ )
//		{
//   		    NAND_ERR((_T("[%02x] : "), i*16));
//		    for ( j = 0; j < 16/4; j++ )
//		    {
//    		    NAND_ERR((_T(" %02x"), (((pSpareCxt[i*16+j])>>0)&0xFF)));
//    		    NAND_ERR((_T(" %02x"), (((pSpareCxt[i*16+j])>>4)&0xFF)));
//    		    NAND_ERR((_T(" %02x"), (((pSpareCxt[i*16+j])>>8)&0xFF)));
//    		    NAND_ERR((_T(" %02x"), (((pSpareCxt[i*16+j])>>12)&0xFF)));
//    		}
//   		    NAND_ERR((_T("\r\n")));
//		}
...
Write_Spare_Separate(UINT32 nBank, UINT32 nPpn, pSECCCxt pSpareCxt)
{
	...
	if (IS_CHECK_SPARE_ECC == TRUE32)
	{
		...
		pSpareCxt->aSECC[3] = NF_8MECC3() & 0xff;
	}
	...
	if (IS_CHECK_SPARE_ECC == TRUE32)
	{
		...
		pSpareCxt->aSECC[7] = NF_8MECC3() & 0xff;
	}
	...
	else
	{
		...
		for ( nLoop = 0 ; nLoop < nErrorCnt && nLoop < 8 ; nLoop++ )
		{
			if (nErrorByte[nLoop] < nEffectiveByte)
			{
				...
				pBuf[nErrorByte[nLoop]] = pBuf[nErrorByte[nLoop]]^nErrorPattern[nLoop];
				NAND_ERR((_T("[FIL] Decoding_Spare8BitECC() : %dth Error Buf[%d] [%02x]->[%02x]\r\n"), nLoop, nErrorByte[nLoop], cTempBuf, pBuf[nErrorByte[nLoop]]));
				nEffectiveAreaErrorCnt ++;
			}
			else if (nErrorByte[nLoop]< 512)
			{
				NAND_ERR((_T("[FIL] Decoding_Spare8BitECC() : %dth Error in Dummy Data Buf[%d]\r\n"), nLoop, nErrorByte[nLoop]));
				bDummyError = TRUE32;
			}		
			else
			{
				NAND_ERR((_T("[FIL] Decoding_Spare8BitECC() : %dth Error in Spare Are[%d]\r\n"), nLoop, nErrorByte[nLoop]));
			}		
		}
		...
		{
			NAND_ERR((_T("[FIL] Decoding_Spare8BitECC() : Correctable Error %d bits\r\n"), nErrorCnt));
#if (PROVE_ECC_ALGORITHM)
            if ( nEffectiveAreaErrorCnt > 0 )
            {
    		    nRet |= ECC_NEED_DECODING_AGAIN;
